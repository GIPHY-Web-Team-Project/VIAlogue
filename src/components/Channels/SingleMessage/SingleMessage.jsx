                ...
                <div className="flex-1 flex-col pr-2 ">
                    ...
                    <div className="flex flex-row justify-between items-start mt-2">
                        ...
                        {msg.reactions && Object.values(msg.reactions).some(users => users.length > 0) && (
                            <div className="flex flex-row gap-2 bg-gray-700 rounded-md px-2 py-1 mt-1">
                                {Object.entries(msg.reactions).map(([emoji, users]) => (
                                    users.length > 0 && (
                                        <>
                                            <button key={emoji} className="flex items-center text-white px-2" onClick={() => setShowUsers(!showUsers)}>
                                                {emoji} {users.length}
                                            </button>
                                            { showUsers && (
